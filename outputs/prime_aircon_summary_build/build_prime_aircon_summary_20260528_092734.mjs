...
const timestamp = "20260528_092734";
...
const commonFeatureRows = [
  ["Gas Ecológico R410a", "R410a；源文件说明不使用氯，强调保护臭氧层、低噪和热泵可靠性。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Recubrimiento Anticorrosivo GoldShield", "抗腐蚀涂层；源文件描述适用于腐蚀、潮湿、盐雾环境，并标注提升制冷速度。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Función Turbo", "快速制冷；源文件描述通过最大化出风量，使房间更快降温。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Alta eficiencia y tecnología", "高技术压缩机、旋转式密闭压缩机、制冷更快、低噪、寿命更长。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Modo Dormir Sleep", "低噪、自动降低风扇速度、关闭显示灯，面向睡眠场景。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Control de Temperatura y Humedad", "面向舒适温湿度控制。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Función I FEEL / Yo siento", "遥控器内置温度传感器，按遥控器附近环境温度调节运行。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Temporizador", "自动开关机定时。", "ELITE 4 / TRENDY K / TRENDY 3"],
  ["Auto Limpieza", "一键自清洁，源文件描述通过蒸发器快速冷却帮助剥离污垢。", "ELITE 4 / TRENDY K / TRENDY 3"],
];
...
const coreRows = allRows.map((r, idx) => [
  ...
  r.sourceFile, r.sourceUrl, r.sourceNote ?? "",
]);
...
const extendedRows = allRows.map((r) => [
  ...
  r.maxPipeLengthM ?? "", r.maxLevelDiffM ?? "", r.sourceFile, r.sourceNote ?? "",
]);
...
const sourceRows = [
  ...
  ["Elite 2 图片", urls.elite2Image, "Elite 2 核心参数来源", "图中字段原文为 EEER/SEEER，本次规范列记为 EER/SEER。"],
  ["Elite 2 型号补充 PDF", urls.elite2ModelPdf, "Elite 2 型号补充", "保留为补充来源。"],
  ["ELITE_4_POPUP.pdf", urls.elite4, "Elite 4 参数与型号", "122-E4 的冷媒充注量用渲染页核对：单冷 0.45 kg，冷暖 0.65 kg。"],
  ["TRENDY_K_POPUP.pdf", urls.trendyK, "Trendy K 参数与型号", "362-K 的 SEER=2.2、低压=50 MPa、室内低档噪音=58 dB(A) 均按 PDF 可见值保留，疑似源文件异常。"],
  ["TRENDY_3_POPUP.pdf", urls.trendy3, "Trendy 3 参数与型号", "12K 表头为 122-T3，但型号格显示 EMPRC/EMPRN/CMPRC/CMPRN121-T3，按源文件保留并标注。"],
  ["采集时间", timestamp, "文件命名与审计", "Asia/Shanghai；本次新建文件，不覆盖旧文件。"],
];
...
setValues(core, `A1:AL1`, [["Prime 空调参数汇总（Elite 2 + Elite 4 + Trendy K + Trendy 3）", ...Array(coreHeaders.length - 1).fill("")]]);
core.getRange("A1:AL1").merge();
styleTitle(core, "A1:AL1");
setValues(core, `A2:AL2`, [["本文件为新建汇总，不覆盖上次文件；疑似源文件异常已保留原值并在备注/来源页说明。", ...Array(coreHeaders.length - 1).fill("")]]);
core.getRange("A2:AL2").merge();
styleSubtitle(core, "A2:AL2");
setValues(core, `A4:AL${4 + coreRows.length}`, [coreHeaders, ...coreRows]);
styleHeader(core, "A4:AL4");
addTable(core, `A4:AL${4 + coreRows.length}`, "PrimeAirconCore");
core.freezePanes.freezeRows(4);
setWidths(core, { A: 55, B: 80, C: 95, D: 95, E: 80, F: 110, G: 145, H: 85, I: 95, J: 95, K: 85, L: 115, M: 110, N: 95, O: 95, P: 80, Q: 95, R: 90, S: 90, T: 125, U: 105, V: 135, W: 135, X: 105, Y: 105, Z: 115, AA: 100, AB: 120, AC: 100, AD: 80, AE: 80, AF: 125, AG: 125, AH: 125, AI: 125, AJ: 185, AK: 300, AL: 330 });
core.getRange("A:AL").format.wrapText = true;

setValues(ext, `A1:AW1`, [["PDF 扩展参数明细", ...Array(extendedHeaders.length - 1).fill("")]]);
ext.getRange("A1:AW1").merge();
styleTitle(ext, "A1:AW1");
setValues(ext, `A2:AW2`, [["按一机型一行保留更细字段；Elite 2 仅含上次图片可得字段，PDF 扩展项为空。", ...Array(extendedHeaders.length - 1).fill("")]]);
ext.getRange("A2:AW2").merge();
styleSubtitle(ext, "A2:AW2");
setValues(ext, `A4:AW${4 + extendedRows.length}`, [extendedHeaders, ...extendedRows]);
styleHeader(ext, "A4:AW4");
addTable(ext, `A4:AW${4 + extendedRows.length}`, "PrimeAirconExtended");
ext.freezePanes.freezeRows(4);
setWidths(ext, { A: 95, B: 95, C: 80, D: 110, E: 110, F: 150, G: 95, H: 95, I: 95, J: 75, K: 95, L: 95, M: 95, N: 75, O: 90, P: 85, Q: 85, R: 85, S: 85, T: 85, U: 85, V: 90, W: 90, X: 95, Y: 115, Z: 95, AA: 95, AB: 135, AC: 135, AD: 135, AE: 135, AF: 100, AG: 100, AH: 100, AI: 100, AJ: 120, AK: 120, AL: 165, AM: 165, AN: 75, AO: 75, AP: 165, AQ: 95, AR: 95, AS: 95, AT: 95, AU: 95, AV: 180, AW: 330 });
ext.getRange("A:AW").format.wrapText = true;
...
setValues(features, "A1:C1", [["功能卖点", "", ""]]);
features.getRange("A1:C1").merge();
styleTitle(features, "A1:C1");
setValues(features, "A2:C2", [["来自三份 2025/06 Prime PDF 首页文案，适合后续做卖点标签。", "", ""]]);
features.getRange("A2:C2").merge();
styleSubtitle(features, "A2:C2");
setValues(features, `A4:C${4 + commonFeatureRows.length}`, [["功能/卖点", "整理说明", "适用系列"], ...commonFeatureRows]);
styleHeader(features, "A4:C4");
addTable(features, `A4:C${4 + commonFeatureRows.length}`, "PrimeAirconFeatures");
features.freezePanes.freezeRows(4);
setWidths(features, { A: 230, B: 620, C: 260 });
features.getRange("A:C").format.wrapText = true;

setValues(sources, "A1:D1", [["来源与口径说明", "", "", ""]]);
sources.getRange("A1:D1").merge();
styleTitle(sources, "A1:D1");
setValues(sources, "A2:D2", [["用于审计来源、记录异常值和合并策略。", "", "", ""]]);
...
addTable(sources, `A4:D${3 + sourceRows.length}`, "PrimeAirconSources");
sources.freezePanes.freezeRows(4);
setWidths(sources, { A: 180, B: 560, C: 300, D: 820 });
...
const renderTargets = [
  ["汇总_核心参数", "A1:AL19", "render_core.png"],
  ["扩展参数_PDF", "A1:AW19", "render_extended.png"],
  ["来源与口径", "A1:D12", "render_sources.png"],
];
