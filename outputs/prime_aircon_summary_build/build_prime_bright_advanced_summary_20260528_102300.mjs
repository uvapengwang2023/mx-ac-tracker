...
const timestamp = "20260528_102300";
...
const extendedHeaders = [
  ...
  "蒸发器管路", "冷凝器管路", "室内风量_m3/h", "室内风扇类型", "制冷风速_rpm", "制热风速_rpm", "干燥风速_rpm",
...
const extendedRows = rows.map((r) => [
  ...
  r.evapPipe ?? "", r.condPipe ?? "", r.indoorAirflowM3h ?? "", r.indoorFanType ?? "", r.indoorFanSpeedCoolingRpm ?? "", r.indoorFanSpeedHeatingRpm ?? "", r.indoorFanSpeedDryRpm ?? "",
...
const sourceRows = [
  ["来源", "URL/文件", "网站录入年月_链接", "图片EXIF时间", "本次用途", "口径说明"],
...
const ext = workbook.worksheets.add("扩展参数_图片");
...
buildSheet(
  core,
  "Prime 空调参数汇总（A1 Advanced + Bright 1 + R3 Advanced + Bright 3）",
  "四个产品来源汇总在一起；链接中的网站录入年月已结构化保存，简版图缺失字段留空并备注。",
  coreHeaders,
  coreRows,
...
buildSheet(
  ext,
  "图片扩展参数明细",
  "保留图片可见的细字段；Bright 1 为简版图，扩展项为空值较多。",
  extendedHeaders,
  extendedRows,
...
sources.getRange("5:10").format.rowHeightPx = 48;
anomalies.getRange("5:8").format.rowHeightPx = 58;
...
const coreInspect = await workbook.inspect({
  kind: "table",
  range: `汇总_核心参数!A4:${colName(coreHeaders.length)}19`,
  include: "values",
  tableMaxRows: 18,
  tableMaxCols: 40,
});
...
const renderTargets = [
  ["汇总_核心参数", `A1:${colName(coreHeaders.length)}19`, "render_new_core.png"],
  ["扩展参数_图片", `A1:${colName(extendedHeaders.length)}19`, "render_new_extended.png"],
  ["功能卖点", "A1:F16", "render_new_features.png"],
  ["异常检查", "A1:G9", "render_new_anomalies.png"],
];
