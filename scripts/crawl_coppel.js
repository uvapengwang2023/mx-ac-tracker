...
const DEFAULT_LONG_BREAK_EVERY_LISTING_PAGES = 2;
const DEFAULT_LONG_BREAK_EVERY_DETAIL_PRODUCTS = 15;
...
function parseArgs(argv) {
  const args = {
    ...
    longBreakEveryListingPages: DEFAULT_LONG_BREAK_EVERY_LISTING_PAGES,
    longBreakEveryDetailProducts: DEFAULT_LONG_BREAK_EVERY_DETAIL_PRODUCTS,
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === "--base-url" && next) {
      ...
      i += 1;
    } else if (token === "--listing-long-break-every" && next) {
      args.longBreakEveryListingPages = Number(next);
      i += 1;
    } else if (token === "--detail-long-break-every" && next) {
      args.longBreakEveryDetailProducts = Number(next);
      i += 1;
      ...
      args.longBreakEveryListingPages = 0;
      args.longBreakEveryDetailProducts = 0;
  ...
  if (
    !Number.isFinite(args.longBreakEveryListingPages) ||
    args.longBreakEveryListingPages < 0
  ) {
    throw new Error("--listing-long-break-every must be a non-negative number");
  }
  if (
    !Number.isFinite(args.longBreakEveryDetailProducts) ||
    args.longBreakEveryDetailProducts < 0
  ) {
    throw new Error("--detail-long-break-every must be a non-negative number");
  }
...
async function main() {
    ...
    for (let pageNumber = 0; pageNumber < pagesToVisit; pageNumber += 1) {
      ...
      if ((pageNumber + 1) < pagesToVisit) {
        const shouldLongBreak =
          args.longBreakEveryListingPages > 0 &&
          (pageNumber + 1) % args.longBreakEveryListingPages === 0;
    ...
    if (args.crawlDetails && allRows.length > 0) {
      ...
      for (let index = 0; index < rowsForDetail.length; index += 1) {
        ...
        if ((index + 1) < rowsForDetail.length) {
          const shouldLongBreak =
            args.longBreakEveryDetailProducts > 0 &&
            (index + 1) % args.longBreakEveryDetailProducts === 0;
          const pauseMs = shouldLongBreak
            ? randomIntInclusive(args.longBreakMinMs, args.longBreakMaxMs)
            : randomIntInclusive(args.minDelayMs, args.maxDelayMs);
          await detailPage.waitForTimeout(pauseMs);
        }
      }
  ...
  const runSummary = {
    ...
    safety: {
      ...
      listing_long_break_every_pages: args.longBreakEveryListingPages,
      detail_long_break_every_products: args.longBreakEveryDetailProducts,
