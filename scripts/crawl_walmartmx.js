...
const DEFAULT_FAILURE_LOG_MAX_ENTRIES = 20;
...
async function appendRollingFailureLog({
  runsDir,
  runSummary,
  maxEntries = DEFAULT_FAILURE_LOG_MAX_ENTRIES,
}) {
  if (!runSummary?.failure) {
    return { appended: false, reason: "no_failure" };
  }

  const logPath = path.join(runsDir, "failure_log.json");
  let existing = [];
  ...
  const entry = {
    ...
    captured_at: runSummary.captured_at || null,
    failure: runSummary.failure || null,
    final_url: runSummary.final_url || null,
    manual_verification: runSummary.manual_verification || null,
    pagination: runSummary.pagination
      ? {
          pages_crawled: runSummary.pagination.pages_crawled ?? 0,
          stop_reason: runSummary.pagination.stop_reason || null,
        }
      : null,
    safety: runSummary.safety || null,
  };

  const merged = [...existing, entry];
  const capped = merged.slice(-Math.max(1, Number(maxEntries) || DEFAULT_FAILURE_LOG_MAX_ENTRIES));
  await fs.writeFile(logPath, JSON.stringify(capped, null, 2), "utf8");

  return { appended: true, logPath, entries: capped.length };
}
...
async function launchChromeForCrawl(args) {
  ...
  const onSpawnError = (error) => {
    if (startupSettled || !startupReject) {
      return;
    }
    startupSettled = true;
    startupReject(error);
  };
  const onSpawnExit = (code, signal) => {
    if (startupSettled || !startupReject) {
      return;
    }
    startupSettled = true;
    startupReject(
      new Error(
        `Chrome process exited before CDP became ready (code=${code ?? "null"}, signal=${signal || "none"})`,
      ),
    );
  };
  child.once("error", onSpawnError);
  child.once("exit", onSpawnExit);

  let endpoint;
  try {
    endpoint = await Promise.race([waitForCdpReady(port, 15000), startupErrorPromise]);
  } catch (error) {
    throw new Error(`Failed to start Chrome CDP session: ${error?.message || error}`);
  } finally {
    startupSettled = true;
    startupReject = null;
    // Remove startup race listeners to avoid dangling handlers.
    child.off("error", onSpawnError);
    child.off("exit", onSpawnExit);
  }
...
async function main() {
  ...
  try {
    ...
    if (!manual.passed || blockedAfter) {
      ...
      await appendRollingFailureLog({ runsDir, runSummary });
      process.exitCode = 3;
      return;
    }
    ...
    const errText = String(error?.message || error || "unknown_error");
    if (!runSummary.failure) {
      runSummary.failure = { reason: "unhandled_exception", error: errText };
    } else if (!runSummary.failure.error) {
      runSummary.failure.error = errText;
    }
    if (!runSummary.pagination.stop_reason) {
      runSummary.pagination.stop_reason = runSummary.failure.reason || "unhandled_exception";
    }
    if (!runSummary.final_url && page) {
      try {
        runSummary.final_url = sanitizeUrlForLog(page.url());
      } catch {
        // ignore
    ...
    try {
      ...
      // ignore failure log write errors
    }
